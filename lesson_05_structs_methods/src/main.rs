...
fn main() {
    ...
    println!("RGB channels: ({}, {}, {})", favorite.0, favorite.1, favorite.2);

    let user = build_user(String::from("Ava"), 3);
    println!("User summary: {} has completed {} lessons.", user.name, user.completed_lessons);
}
...
fn build_user(name: String, completed_lessons: u8) -> UserProgress {
    UserProgress { name, completed_lessons }
}
